...
def main():
    # 1. Create compiler + AutoDictionary
    hc = SanskritMeshCompiler(level="hyper")
    ...
    auto.apply(hc)
    print(f"Dictionary size now: {len(hc.dictionary)} entries")
    ...
    hc_no_learn = SanskritMeshCompiler(level="hyper")
    v1_result = hc_no_learn.compile_payload(test_payload)
    v1_str = json.dumps(v1_result, separators=(",", ":"), ensure_ascii=False)
    print(f"\nBefore learning: {len(v1_str)} chars")

    # WITH auto-learning
    v2_compressed = hc.compile_payload(test_payload)
    restored = hc.decompile_payload(v2_compressed)
    v2_str = json.dumps(v2_compressed, separators=(",", ":"), ensure_ascii=False)
